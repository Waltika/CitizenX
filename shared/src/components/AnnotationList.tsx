...
export const AnnotationList: React.FC<AnnotationListProps> = ({ annotations, profiles, onDelete, onSaveComment }) => {
    ...
    const quillInstances = useRef<Record<string, any | null>>({});
    ...
    const handleShare = async (annotation: AnnotationType) => {
                        ...
                        {sortedComments.length > 0 && (
                            <>
                                <button
                                    ...
                                    data-comment-count={sortedComments.length}
                                    onClick={() => toggleComments(annotation.id)}
                                >
                                    {isExpanded ? '− Hide comments' : `+ Show ${sortedComments.length} comment${sortedComments.length > 1 ? 's' : ''}`}
                                </button>
                                {isExpanded && (
