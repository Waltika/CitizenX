...
export const useUserProfile = (): UseUserProfileReturn => {
    ...
    const exportIdentity = useCallback(async (passphrase: string) => {
        ...
        try {
            ...
            throw new Error('Failed to export identity: ' + ((err as Error).message || 'Unknown error'));
        }
    ...
    const importIdentity = useCallback(async (data: string, passphrase: string) => {
        ...
        try {
            ...
            if ((err as Error).message.includes('OperationError') || (err as Error).message.includes('Failed to decrypt')) {
                errorMessage = 'Failed to import identity: Incorrect passphrase or corrupted data';
            } else if ((err as Error).message.includes('Invalid DID format')) {
                errorMessage = 'Failed to import identity: Invalid DID format';
            } else {
                errorMessage = 'Failed to import identity: ' + ((err as Error).message || 'Unknown error');
            }
