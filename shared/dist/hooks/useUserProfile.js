...
export const useUserProfile = () => {
            ...
            try {
                ...
                if (storedDid) {
                    if (validateDID(storedDid)) {
                        ...
                        const storedPrivateKey = await new Promise((resolve) => {
    ...
    const exportIdentity = useCallback(async (passphrase) => {
        if (storageLoading) {
    ...
    const importIdentity = useCallback(async (data, passphrase) => {
        if (storageLoading) {
            throw new Error('Storage is still initializing');
        }
        if (!storage) {
    ...
    const createProfile = useCallback(async (handle, profilePicture) => {
        if (storageLoading) {
            setError('Storage is still initializing');
            return;
        }
    ...
    const updateProfile = useCallback(async (handle, profilePicture) => {
        if (storageLoading) {
            setError('Storage is still initializing');
            return;
        }
