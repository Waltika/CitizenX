...
export async function exportKeyPair(data, passphrase) {
    try {
        const passphraseKey = await crypto.subtle.importKey('raw', stringToArrayBuffer(passphrase), { name: 'PBKDF2' }, false, ['deriveBits', 'deriveKey']);
        ...
        const encryptedData = await crypto.subtle.encrypt({
            name: 'AES-GCM',
            iv,
        }, derivedKey, stringToArrayBuffer(data));
        const exportData = {
...
export async function importKeyPair(encryptedData, passphrase) {
    try {
...
export async function verifySignature(did, message, signatureHex) {
    try {
